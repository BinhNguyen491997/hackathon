...
```bash
bash .kiro/skills/agentbase/scripts/aip.sh api-keys list [--name NAME] [--page N] [--size N]
```
...
```bash
bash .kiro/skills/agentbase/scripts/aip.sh api-keys create --name my-agent-key [--default]
```
...
> - **API Key:** saved to `.env` as `LLM_API_KEY`. Verify with `bash .kiro/skills/agentbase/scripts/check_credentials.sh llm`

### api-keys get [name]
Get details of a specific API key. The `key` field is redacted in stdout by design — the plaintext key is never printed.

```bash
bash .kiro/skills/agentbase/scripts/aip.sh api-keys get my-agent-key
```
...
```bash
bash .kiro/skills/agentbase/scripts/aip.sh api-keys get my-agent-key --save-env
# then verify: bash .kiro/skills/agentbase/scripts/check_credentials.sh llm
```
...
```bash
bash .kiro/skills/agentbase/scripts/aip.sh api-keys update my-agent-key --default true
```
...
```bash
bash .kiro/skills/agentbase/scripts/aip.sh api-keys delete my-agent-key
```
...
```bash
# List all models
bash .kiro/skills/agentbase/scripts/aip.sh models list

# Filter by provider and type
bash .kiro/skills/agentbase/scripts/aip.sh models list --providers openai --types chat --status ENABLED
```

> **Tip**: Use `bash .kiro/skills/agentbase/scripts/aip.sh models metadata` to discover valid filter values for `--providers`, `--types`, and `--status`.
...
```bash
bash .kiro/skills/agentbase/scripts/aip.sh models get MODEL_UUID
```
...
```bash
bash .kiro/skills/agentbase/scripts/aip.sh models metadata
```
...
```bash
bash .kiro/skills/agentbase/scripts/aip.sh models enable MODEL_UUID
```
...
```bash
bash .kiro/skills/agentbase/scripts/aip.sh models disable MODEL_UUID
```
...
```bash
bash .kiro/skills/agentbase/scripts/aip.sh models rate-limit MODEL_UUID
```
