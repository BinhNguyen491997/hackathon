...
```bash
bash .kiro/skills/agentbase/scripts/discovery.sh json
```
...
Delete in this specific order to avoid dependency errors. If any script call returns an auth error (401/403) during the teardown sequence, re-authenticate with `bash .kiro/skills/agentbase/scripts/get_token.sh --force` and retry the failed call once. If the retry also fails, report the error and continue with the next deletion.
...
```bash
# API Key providers
bash .kiro/skills/agentbase/scripts/auth.sh apikey delete --name $NAME

# Delegated providers
bash .kiro/skills/agentbase/scripts/auth.sh delegated delete --name $NAME

# OAuth2 providers
bash .kiro/skills/agentbase/scripts/auth.sh oauth2 delete --name $NAME
```

**Phase 4 — Agent identity:**
```bash
bash .kiro/skills/agentbase/scripts/identity.sh delete $NAME
```

**Phase 5 — Memory:**
```bash
bash .kiro/skills/agentbase/scripts/memory.sh delete $MEMORY_ID
```
...
PAGE=1
while true; do
  RESULT=$(bash .kiro/skills/agentbase/scripts/cr.sh images list --name $PROJECT_NAME --page $PAGE --size 100)
  # For each image in $RESULT.data[].name:
  #   bash .kiro/skills/agentbase/scripts/cr.sh images delete --name $IMAGE_NAME
...
bash .kiro/skills/agentbase/scripts/cr.sh images list --name $PROJECT_NAME
```

**Phase 7 — AIP API keys** (optional, may be shared):
```bash
bash .kiro/skills/agentbase/scripts/aip.sh api-keys delete $KEY_NAME
```
...
4. Discover all related resources using `bash .kiro/skills/agentbase/scripts/discovery.sh json`.
