...
All operations use the runtime script: `bash .kiro/skills/agentbase/scripts/runtime.sh`
...
bash .kiro/skills/agentbase/scripts/vserver.sh projects

# 2. List VPCs in the project — filtered to id, name, cidr, status, dnsStatus
bash .kiro/skills/agentbase/scripts/vserver.sh vpcs <projectId>
...
bash .kiro/skills/agentbase/scripts/vserver.sh validate-vpc <projectId> <vpcId>
```
...
```bash
bash .kiro/skills/agentbase/scripts/runtime.sh list --page 1 --size 20
```
...
bash .kiro/skills/agentbase/scripts/runtime.sh endpoints delete $RUNTIME_ID $CUSTOM_ENDPOINT_ID

# 2. Then delete the runtime
bash .kiro/skills/agentbase/scripts/runtime.sh delete $RUNTIME_ID
```
...
**Create endpoint**:
```bash
bash .kiro/skills/agentbase/scripts/runtime.sh endpoints create $RUNTIME_ID \
  --name canary --version 2
```

**Update endpoint version**:
```bash
bash .kiro/skills/agentbase/scripts/runtime.sh endpoints update $RUNTIME_ID $ENDPOINT_ID \
  --version 3
```

**Delete endpoint**:
```bash
bash .kiro/skills/agentbase/scripts/runtime.sh endpoints delete $RUNTIME_ID $ENDPOINT_ID
```

**Start / stop endpoint** (lifecycle control — stop halts the endpoint from serving traffic without deleting it; start resumes it):
```bash
bash .kiro/skills/agentbase/scripts/runtime.sh endpoints stop  $RUNTIME_ID $ENDPOINT_ID
bash .kiro/skills/agentbase/scripts/runtime.sh endpoints start $RUNTIME_ID $ENDPOINT_ID
```
...
```bash
bash .kiro/skills/agentbase/scripts/runtime.sh versions $RUNTIME_ID --page 1 --size 20
```
...
while true; do
  STATUS=$(bash .kiro/skills/agentbase/scripts/runtime.sh get $RUNTIME_ID | jq -r '.status')
  echo "Status: $STATUS"
...
```bash
bash .kiro/skills/agentbase/scripts/runtime.sh reset-service-account $RUNTIME_ID
```
...
```bash
bash .kiro/skills/agentbase/scripts/runtime.sh flavors
```
