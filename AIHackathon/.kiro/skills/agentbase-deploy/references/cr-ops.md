...
All operations use the CR script: `bash .kiro/skills/agentbase/scripts/cr.sh`
...
bash .kiro/skills/agentbase/scripts/cr.sh repo get
...
bash .kiro/skills/agentbase/scripts/cr.sh credentials get

# Show the secret in plaintext (only when you need it, e.g. for manual docker login)
REDACT_FIELDS= bash .kiro/skills/agentbase/scripts/cr.sh credentials get
```

### Rotate (reset) credentials
```bash
bash .kiro/skills/agentbase/scripts/cr.sh credentials reset
```
...
```bash
bash .kiro/skills/agentbase/scripts/cr.sh credentials docker-login --reset
```
...
bash .kiro/skills/agentbase/scripts/cr.sh images list --page 1 --size 50

# Filter by name substring
bash .kiro/skills/agentbase/scripts/cr.sh images list --name myagent
```
...
```bash
bash .kiro/skills/agentbase/scripts/cr.sh images delete --name myagent
```
...
```bash
bash .kiro/skills/agentbase/scripts/cr.sh artifacts list --image myagent --page 1 --size 50
```
...
```bash
bash .kiro/skills/agentbase/scripts/cr.sh artifacts list --image myagent --digest sha256:abc123...
```

### Delete an artifact
```bash
bash .kiro/skills/agentbase/scripts/cr.sh artifacts delete \
...
bash .kiro/skills/agentbase/scripts/cr.sh artifacts list --image myagent --size 100
...
|---------|--------------|-----|
| `401 Unauthorized` on `cr.sh` calls | IAM token missing/expired | Run `bash .kiro/skills/agentbase/scripts/check_credentials.sh iam` |
