...
bash .kiro/skills/agentbase/scripts/runtime.sh logs $RUNTIME_ID --from 0 --limit 100

# With time range and keyword search
bash .kiro/skills/agentbase/scripts/runtime.sh logs $RUNTIME_ID \
  --from 0 --limit 100 \
...
**Command**:
```bash
bash .kiro/skills/agentbase/scripts/runtime.sh endpoints logs $RUNTIME_ID $ENDPOINT_ID \
  --from 0 --limit 100
```
...
```bash
# Current metrics
bash .kiro/skills/agentbase/scripts/runtime.sh endpoints metrics $RUNTIME_ID $ENDPOINT_ID
...
**Command**:
```bash
bash .kiro/skills/agentbase/scripts/runtime.sh endpoints events $RUNTIME_ID $ENDPOINT_ID
```
...
bash .kiro/skills/agentbase/scripts/runtime.sh traces search --param key=value [--param key=value ...]

# Get a single trace by ID
bash .kiro/skills/agentbase/scripts/runtime.sh traces get $TRACE_ID [--param key=value ...]

# List available values for a trace tag key (for building filters)
bash .kiro/skills/agentbase/scripts/runtime.sh traces tag-values $TAG_KEY [--param key=value ...]
```
...
for i in $(seq 1 $MAX_POLLS); do
  RESULT=$(bash .kiro/skills/agentbase/scripts/runtime.sh logs $RUNTIME_ID --from $OFFSET --limit $LIMIT)
...
bash .kiro/skills/agentbase/scripts/runtime.sh logs $RUNTIME_ID --from 0 --limit 100 --query "error"
...
LOGS=$(bash .kiro/skills/agentbase/scripts/runtime.sh logs $RUNTIME_ID --from 0 --limit 100)
...
1. **Discover all resources** using `bash .kiro/skills/agentbase/scripts/discovery.sh`
...
2. **For logs/metrics/events operations:**
   a. If a runtime ID is needed and not provided, list runtimes first (`bash .kiro/skills/agentbase/scripts/runtime.sh list`) and ask the user to pick one.
   b. If an endpoint ID is needed, list endpoints for the runtime (`bash .kiro/skills/agentbase/scripts/runtime.sh endpoints list $RUNTIME_ID`) and ask the user to pick one.
...
4. **For dashboard operation:**
   ...
   b. Run `bash .kiro/skills/agentbase/scripts/discovery.sh` (or `bash .kiro/skills/agentbase/scripts/discovery.sh json` for raw JSON).
