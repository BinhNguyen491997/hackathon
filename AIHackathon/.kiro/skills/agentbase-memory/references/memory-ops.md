...
**Script**: `bash .kiro/skills/agentbase/scripts/memory.sh` (handles authentication, token refresh, response redaction, and error handling automatically)
...
**CLI:**
```bash
bash .kiro/skills/agentbase/scripts/memory.sh create \
  --name my-memory \
...
**CLI:**
```bash
bash .kiro/skills/agentbase/scripts/memory.sh list --page 1 --size 10
```
...
bash .kiro/skills/agentbase/scripts/memory.sh get mem_abc123

# Get long-term memory strategies
bash .kiro/skills/agentbase/scripts/memory.sh strategies mem_abc123
```
...
**CLI:**
```bash
bash .kiro/skills/agentbase/scripts/memory.sh delete mem_abc123
```
...
**CLI:**
```bash
bash .kiro/skills/agentbase/scripts/memory.sh events list mem_abc123 user-1 session-1 --page 1 --size 20
```
...
**CLI (conversational):**
```bash
bash .kiro/skills/agentbase/scripts/memory.sh events create mem_abc123 user-1 session-1 \
...
**CLI:**
```bash
bash .kiro/skills/agentbase/scripts/memory.sh actors mem_abc123
```
...
**CLI:**
```bash
bash .kiro/skills/agentbase/scripts/memory.sh records list mem_abc123 \
...
**CLI:**
```bash
bash .kiro/skills/agentbase/scripts/memory.sh records search mem_abc123 \
...
**CLI:**
```bash
bash .kiro/skills/agentbase/scripts/memory.sh records generate-from-session mem_abc123 \
