...
Use the token script: `TOKEN=$(bash .kiro/skills/agentbase/scripts/get_token.sh)`. It caches the token in `.agentbase/token_cache` and validates expiry via JWT `exp` claim. On 401: re-run with `--force`.
