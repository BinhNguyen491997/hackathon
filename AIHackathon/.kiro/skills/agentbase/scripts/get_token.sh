...
# Usage: TOKEN=$(bash .kiro/skills/agentbase/scripts/get_token.sh)
...
if [ -z "$token" ]; then
  ...
  if [ -n "$error_msg" ]; then
    ...
    echo "ERROR: Failed to fetch token. Check credentials with: bash .kiro/skills/agentbase/scripts/check_credentials.sh iam" >&2
  fi
  exit 1
fi
